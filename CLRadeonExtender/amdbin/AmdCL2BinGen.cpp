                  ...
                  bool storeLocalSize = false)
{
    ...
    if (config.useArgs)
        userSGPRsNum += 2;
    return (config.pgmRSRC2 & 0xffffe440U) | (userSGPRsNum<<1) |
                ...
                bool is64Bit)
{
    ...
    uint16_t setup1 = 0x1;
    if (config.useGeneric) // This should be renamed to "useflat."
        setup1 |= 0x20;
    if (config.useArgs)
        setup1 |= 0x8;
    if (config.useEnqueue)
    ...
    //archInd = (arch==GPUArchitecture::GCN1_2 && newBinaries) ? 0x40 : archInd;
    if (config.gdsSize)
        archInd |= 1;

    SLEV(setupData.archInd, archInd);
